...
//	float activeCardRightBound;
	...
	void Update() {
		...
		if (isNotDead)
		{
			...
			if ((Input.touchCount > 0 || Input.GetMouseButtonDown(0)) && isTouchOkay)
			{
				...
				if (IsInGridPosition1(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 1&& Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 1 was tapped
				{
					GridPosition1();
				}
				else if (IsInGridPosition2(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 2 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 2 was tapped
				{
					GridPosition2();
				}
				else if (IsInGridPosition3(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 3 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 3 was tapped
				{
					GridPosition3();
				}
				else if (IsInGridPosition4(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 4 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 4 was tapped
				{
					GridPosition4();
				}
				else if (IsInGridPosition5(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 5 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 5 was tapped
				{
					GridPosition5();
				}
				else if (IsInGridPosition6(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 6 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 6 was tapped
				{
					GridPosition6();
				}
				else if (IsInGridPosition7(touchLocation) && ActiveCardQueue.ActiveCardQ.Count >= 7 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 7 was tapped
				{
					GridPosition7();
				}
				else if (IsInGridPosition8(touchLocation) && ActiveCardQueue.ActiveCardQ.Count == 8 && Input.GetTouch(0).phase == TouchPhase.Began)					//Check if Grid position 8 was tapped
				{
					GridPosition8();
				}
				else if ((touchLocation.x < Screen.width/2) && isGrounded && Input.GetTouch(0).phase == TouchPhase.Began)		//Touch left side of screen and grounded, switch gravity direction.
				{
	...
	void ActivateSelection() {
		...
		{
			maxSpeed += 0.15f;
			GetComponent<Rigidbody2D>().mass += 0.24f;
			gravityScale += 0.3f;
			SCoreKeeper.bonusScore += 20;
        }
        else if (numTall == 3)
        {
